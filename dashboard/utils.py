...
def get_all_user():
    try:
        url = "https://crm.deluxebilisim.com/api/staffs/search/"
        HEADERS = {
        ...
        print(f" Error fetching user information")
        # time.sleep(0.3)
    return {}
def get_all_screenshot_paths():
    ...
    return screenshot_map
def convert_browser_date_to_iso(date_string):
    """
    Converts a browser-style date string like:
    'Tue Jun 10 2025 20:40:54 GMT+0500 (Pakistan Standard Time)'
    into '2025-06-10' (YYYY-MM-DD format).
    """
    try:
        ...
        # Handle any format errors or malformed input
        return None
...
def url_to_base64(image_url):
    try:
        if image_url.startswith("http"):
            ...
            response.raise_for_status()  # throws error on 404/403
            image_data = response.content  # this is in bytes
            base64_image = base64.b64encode(image_data).decode('utf-8')
            return base64_image
        else:
            print(f"[Invalid URL] Skipping: {image_url}")
            return None
    except Exception as e:
        print(f"[ERROR] Failed to fetch: {image_url} — {e}")
        return None
    
def analyze_screenshot(image_url):
    try:
        print('inside of the analyzer')
        ...
        messages=[
                {
                    "role": "user",
                    "content": [
                        {"type": "text", "text": "In images below, identify which software or website the user was using (like Facebook, YouTube, Chrome, VSCode, etc.) and calculate how long each was used.  Give a clear report only with total usage per software as JSON like {\"Facebook\": \"10 mins\", \"VSCode\": \"30 mins\"}. If fail, return empty object {}."}
                    ] + image_url
                }
            ],
            max_tokens=1000
...
def process_images_in_batches(image_urls, batch_size=5):
    logger.info('--- inside of the process Image ----') 
    final_totals = defaultdict(float)  # software_name -> total_duration
    openai.api_key = "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"
    logger.info(len(image_urls))
        ...
        try:
            response = openai.ChatCompletion.create(
                ...
                messages=[
                    {
                        "role": "system",
                        "content": "You're an assistant that analyzes desktop screenshots for software usage."
                    },
                    {
                        "role": "user",
                        "content": [
            ...
            logger.info('-------------CHECKING before and after')
            content = response.choices[0].message.content
            logger.info(content)
            batch_result = extract_json_from_content(content)
            logger.info(batch_result) 
            logger.info('------------- after')
            # Assuming response is JSON like: {"Facebook": "10 mins", "VSCode": "30 mins"}
            
            # batch_result = json.loads(content)
            
            for software, duration in batch_result.items():
                try:
                    # Simple parsing: remove " mins" and convert to float
                    duration_clean = duration.replace(' mins', '').replace(' min', '').strip()
                    minutes = float(duration_clean)
                    final_totals[software] += minutes
            
                except Exception as e:
                    logger.info(f"Batch {i//batch_size + 1} failed: {e}")
    ...
    # Convert final result to dict with "X mins" strings
    final_result = {software: f"{minutes} mins" for software, minutes in final_totals.items()}
    return final_result
