...
from .ai_helpers import normalize_task_name_with_ai, get_latest_screenshot_paths
# from .crm_status_data import get_status_counts
from django.shortcuts import render
from . import crm_status_data
from .crm_status_data import get_timesheets, get_staff_details, determine_status
from . import crm_status_data as crm_api
import json
import hashlib
from pathlib import Path
from django.http import JsonResponse
...
def get_status_counts():
    """
    Function to load status data from the JSON file and count occurrences of each status.
    Returns a dictionary with the count of 'Working', 'Break', 'Idle', 'Off'.
    """
    try:
        # Load JSON data
        ...
        return {"Working": 0, "Break": 0, "Idle": 0, "Off": 0}  # Return default counts if file not found
    except json.JSONDecodeError:
        print("⚠️ Error decoding JSON. Returning default status counts.")
        return {"Working": 0, "Break": 0, "Idle": 0, "Off": 0}  # Return default counts if JSON is malformed
    ...
    # Iterate over the data and count the statuses
    for item in data:
        status = item.get("status", "").capitalize()  # Normalize status to match the keys
