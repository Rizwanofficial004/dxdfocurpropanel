...
const Change = styled.div`
  ...
  border-radius: 16px;
  background: ${props => props.type === 'positive' ? '#10b98115' : '#ef444415'};
  color: ${props => props.type === 'positive' ? '#10b981' : '#ef4444'};
`;
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // Try the CRM comprehensive API
        try {
          const crmData = await apiService.getCrmComprehensive();
          console.log('✅ CRM comprehensive data:', crmData);
            ...
            const newStatsData = [
              ...
              },
              {
                ...
                subStats: [
                  ...
                  { label: 'On Hold', value: total_tasks.metrics?.on_hold || 0 }
                ],
        ...
        setStatsData(emergencyFallback);
  ...
  return (
    <CardWrapper>
      {statsData.map((stat, index) => (
        <Card key={index} color={stat.color}>
          <StatusIndicator isLive={dataSource.includes('CRM') || dataSource.includes('Real-time')}>
            {dataSource.includes('CRM') || dataSource.includes('Real-time') ? 'LIVE' : 'CACHE'}
          </StatusIndicator>
